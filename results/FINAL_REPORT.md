...
## v0.12-v0.18 Runtime Milestones
...
| v0.13 INT8 | Per-channel weight-only INT8 MatMul, FP32 accumulation, bias/ReLU, error check | Dense Decoder and MoE Expert W1/W3/W2 paths are covered; reduced-precision attention remains outside |
...
| v0.15 NEON | ARM NEON intrinsic source, AArch64 syntax compile, and QEMU ELF runtime smoke | Native ARM hardware performance remains unvalidated |
...
| v0.17-v0.18 Closure | Dense Decoder INT8 Q/K/V/O/Gate/Up/Down, MoE Expert INT8 W1/W3/W2, prefill and KV-cache Decode; direct paged traversal; AArch64 syntax and QEMU ELF execution | ARM native performance still requires an ARM host |
...
AArch64 NEON syntax compilation and QEMU AArch64 ELF execution from the x86_64 host. The deterministic fuzz
...
- AVX2 is runtime-validated; NEON is AArch64 syntax-validated and QEMU-runtime-
  validated, while AVX-512 and ARM native performance remain unvalidated.
- The machine has one NUMA node, so cross-node performance is not validated.
- Heterogeneous-core placement, NUMA scheduling, block-sparse lowering, and
  distributed expert parallelism are outside this release; MoE Expert INT8 is
  covered for W1/W3/W2.
...
- Dense Decoder Layer and MoE Expert W1/W3/W2 support weight-only INT8 with
  FP32 accumulation. BF16, quantized routing, and reduced-precision attention
  remain future work.
