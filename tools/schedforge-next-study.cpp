...
int main(int argc, char** argv) {
    try {
        ...
        csv << "version,paged_tokens,paged_pages,paged_attention_error,paged_attention_ms,int8_ms,int8_error,decoder_int8_ms,decoder_int8_error,transfer_chunk,transfer_workers,transfer_gbps,neon_compiled,neon_host,neon_cross_compile\n"
            << "0.17.0," << paged.length << ',' << paged.page_table.size() << ','
        ...
        return quantized_result.max_error < 0.05 && decoder_result.max_error < 0.15 &&
            ...
            neon.cross_compile_succeeded ? 0 : 1;
